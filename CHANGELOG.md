...
## v26.12.2（2026-08-29）· 构建脚本改名 .release.ps1

> - `BUILD.ps1` 改名为 `.release.ps1`（git mv 保留历史）；脚本头部用法注释、2 README（目录树/构建章节/签名说明）、CONTRIBUTING、PR 模板中的引用全部同步，目录树对齐修正
> - CHANGELOG 历史记录中的 BUILD.ps1 字样保留原样（描述当时事实，不回溯改写）

## v26.12.2（2026-08-29）· 版本升级 + 全量构建 + kits 测试兜底修复

> - 版本升至 26.12.2（两个 Cargo.toml + Cargo.lock + installer.iss MyAppVersion/KitsVersion，BUILD.ps1 自动注入）；`.\BUILD.ps1 -Upx` 全流程 exit=0：测试 200 + 38/2 全过、64/32 位构建、双插件 UPX、安装包与 UPX 版全部签名（7 产物齐）
> - 修复 kits 测试兜底缺口：`invoke_large` 直连 `kit_bin()` 未走 `invoke()` 的缺失自动构建（bin 改名后 `target\debug` 无 `osmium-kits.exe`，并行测试抢建窗口 NotFound）——抽出 `ensure_built` 公共兜底两处共用；清理 `target\debug` 旧名残留 `osmium-kit.exe`；删除产物模拟全新环境验证兜底路径通过

## v26.12.1（2026-08-29）· 项目记录拆分 CHANGELOG + LICENSE 升级 Apache-2.0 + 新增 NOTICE

> - 项目记录从 CLAUDE.md 拆出为 CHANGELOG.md（CLAUDE.md 只保留项目规则，规则中的记录位置同步更新）
> - LICENSE 由 MIT 整体替换为 Apache-2.0 全文；新增 NOTICE 文件（版权归属 + 第三方依赖标注）；两个 Cargo.toml 补 `license = "Apache-2.0"`
> - 2 README 同步（项目结构树新增 CHANGELOG.md / NOTICE 条目、LICENSE 注明 Apache-2.0、CLAUDE.md 描述改为仅规则、License 章节补 Apache-2.0 声明）
...
> - 实：:①`Save-X86Env`/`Restore-Env` 辅助函数——手动工具链环境（无 vswhere）下切 x86 LIB/INCLUDE/PATH 并设 `CC_i686_pc_windows_msvc` 环境变量（**cc-rs/ring 汇编必须显式指定 x86 cl.exe，否则按 PATH 误选 x64 机器类型导致 LNK1112**）;标准 VS 环境由 rustc/cc-rs 自动处理；②4.3 段构建 i686 target（rustup 自动补装）+ 复制产物 + 签名；③UPX 询问段复用同一环境切换生成 osmium32-upx.exe（opt-level=z）
