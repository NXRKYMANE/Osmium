...
fn split_credential_parses_domain_user() {
    assert_eq!(split_credential("DOMAIN\\alice"), ("DOMAIN".to_string(), "alice".to_string()));
    assert_eq!(split_credential("bob"), ("".to_string(), "bob".to_string()));
    assert_eq!(split_credential("CORP\\a\\b"), ("CORP".to_string(), "a\\b".to_string()));
    assert_eq!(split_credential("user@example.com"), ("".to_string(), "user@example.com".to_string()));
}
...
fn sspi_spn_default_port_omits_and_custom_port_appends() {
    ...
    assert_eq!(sspi_spn("files.corp", "https", Some(443)), "HTTP/files.corp");
    assert_eq!(sspi_spn("files.corp", "http", None), "HTTP/files.corp");
    assert_eq!(sspi_spn("files.corp", "http", Some(8080)), "HTTP/files.corp:8080");
    assert_eq!(sspi_spn("files.corp", "https", Some(8443)), "HTTP/files.corp:8443");
}
...
fn sspi_download_rejects_401_without_negotiate() {
    ...
    let (addr, stop, _count) = spawn_http_server(move |_method, _lines| {
        ("401 Unauthorized".to_string(), vec![], vec![])
    });
    ...
    assert!(matches!(&result, Err(msg) if msg.contains("without Negotiate")),
        "401 无 Negotiate 挑战必须快速报错，实际 {:?}", result);
    let _ = std::fs::remove_file(&tmp);
}
...
fn sspi_download_succeeds_without_auth_challenge() {
    ...
    let (addr, stop, _count) = spawn_http_server(move |_method, _lines| {
        ("200 OK".to_string(), vec![("Content-Length".into(), body2.len().to_string())], body2.clone())
    });
    let url = format!("http://{}:{}", addr.ip(), addr.port());
    ...
    assert_eq!(std::fs::read(&target).unwrap(), body, "文件内容应与服务器一致");
    let _ = std::fs::remove_file(&target);
}
...
fn sspi_download_writes_tmp_atomically_and_renames() {
    ...
    assert!(!std::path::Path::new(&tmp_path).exists(), "下载完成后不得残留 tmp 文件");
...
fn sspi_download_with_explicit_credentials_ok_on_anonymous_server() {
    ...
    let result = sspi_download_to_file(
        &url, target.to_str().unwrap(),
        Some("CORP\\alice"), Some("s3cret"),
        None, 5,
    );
    stop.store(true, Ordering::Relaxed);
    assert!(result.is_ok(), "显式凭据 + 匿名 200 应成功，实际 {:?}", result);
    assert_eq!(std::fs::read(&target).unwrap(), body);
    let _ = std::fs::remove_file(&target);
}
...
fn unzip_to_dir_extracts_and_blocks_traversal() {
    ...
    assert!(!dir.join("evil.txt").exists(), "zip-slip 条目不得写出目标目录");
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn netmap_empty_input_succeeds_and_bad_share_reports_error() {
    ...
    assert!(errors[0].contains("error"), "错误信息应含错误码: {}", errors[0]);
...
fn unknown_kit_fails() {
    ...
    assert_ne!(code, 0);
    assert!(out.contains("unknown kit"), "stdout 应含 unknown kit: {out}");
...
fn netmap_unknown_action_fails() {
    let (code, out, err) = invoke(r#"{"kit":"netmap","action":"frobnicate","mappers":[{"local_path":"Z:","remote_path":"\\\\srv\\share"}]}"#);
    assert_ne!(code, 0);
...
fn unzip_extracts_and_blocks_traversal() {
    ...
    let ok_zip = dir.join("ok.zip");
    {
        ...
        let mut zw = zip::ZipWriter::new(f);
        zw.start_file("sub/data.txt", zip::write::SimpleFileOptions::default()).unwrap();
    ...
    assert_eq!(std::fs::read_to_string(out_dir.join("sub/data.txt")).unwrap(), "protocol payload");
    ...
    assert!(out.contains(r#""ok":false"#));
    assert!(err.contains("osmium-kit error"), "stderr 应抛出错误详情: {err}");
...
fn sspi_download_succeeds_via_local_server() {
    ...
    assert_eq!(std::fs::read(&target).unwrap(), body, "文件内容应与服务器一致");
    assert!(!target.with_extension("download.tmp").exists(), "成功下载后临时文件必须清理");
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn sspi_download_authenticates_against_real_iis() {
    ...
    // 站点要求: 匿名关闭 + Windows 认证启用，绑定 8808 端口，
    // 根目录放 test.txt（内容须与下方断言一致）
