...
use windows::Win32::Security::Authentication::Identity::{
    AcquireCredentialsHandleW, DeleteSecurityContext, FreeContextBuffer, FreeCredentialsHandle, ISC_REQ_ALLOCATE_MEMORY, ISC_REQ_CONFIDENTIALITY,
    ISC_REQ_INTEGRITY, ISC_REQ_MUTUAL_AUTH,
    InitializeSecurityContextW, SECBUFFER_TOKEN, SECPKG_CRED_OUTBOUND,
    SEC_WINNT_AUTH_IDENTITY_EXW, SEC_WINNT_AUTH_IDENTITY_VERSION, SecBuffer, SecBufferDesc,
};
...
pub fn sspi_download_to_file(
    ...
    if let Err(e) = acquire_result {
        return Err(format!("AcquireCredentialsHandleW failed (0x{:08X}). The current process may lack a usable Windows identity.", e.code().0 as u32));
    }
    ...
    for _ in 0..3 {
        ...
        let resp = req.call().map_err(|e| format!("request failed for {url}: {e}"))?;
        if resp.status().is_success() {
            let mut reader = resp.into_body().into_reader();
            std::io::copy(&mut reader, &mut file).map_err(|e| format!("failed to write '{to}': {e}"))?;
            drop(file);
            std::fs::rename(&tmp, to).map_err(|e| format!("rename failed for '{to}': {e}"))?;
            return Ok(());
        }
        if resp.status().as_u16() != 401 {
            return Err(format!("server returned HTTP {} for {url}", resp.status().as_u16()));
        }
        ...
        let challenge = resp.headers()
            ...
            .find(|v| v.to_lowercase().starts_with("negotiate") || v.to_lowercase().starts_with("ntlm"))
            .map(|s| s.to_string());
        ...
        let input = challenge.split_whitespace().nth(1)
        ...
        let input_ptr: Option<*const SecBufferDesc> = if input.is_empty() && guard.ctx.is_none() {
            ...
            input_desc = SecBufferDesc { ulVersion: 0, cBuffers: 1, pBuffers: &mut input_buffer };
        ...
        let mut out_desc = SecBufferDesc { ulVersion: 0, cBuffers: 1, pBuffers: &mut out_buffer };
        let mut attrs: u32 = 0;
        ...
        let status = unsafe {
            InitializeSecurityContextW(
                ...
                ISC_REQ_ALLOCATE_MEMORY | ISC_REQ_CONFIDENTIALITY | ISC_REQ_MUTUAL_AUTH | ISC_REQ_INTEGRITY,
                0, 0,
                input_ptr,
                0,
                Some(&mut new_ctx),
        ...
        let out_token = if out_buffer.pvBuffer.is_null() {
            ...
            Some(unsafe {
                std::slice::from_raw_parts(out_buffer.pvBuffer as *const u8, out_buffer.cbBuffer as usize).to_vec()
            })
        };
        if !out_buffer.pvBuffer.is_null() {
            unsafe { let _ = FreeContextBuffer(out_buffer.pvBuffer); }
        }
        ...
        if let Some(old) = guard.ctx.replace(new_ctx) {
            unsafe { let _ = DeleteSecurityContext(&old); }
        }
...
use windows::Win32::NetworkManagement::WNet::{NETRESOURCEW, NET_CONNECT_FLAGS,
                                              NET_RESOURCE_SCOPE, RESOURCETYPE_DISK,
    ...
    unsafe {
        ...
        for m in mappers {
            ...
            let result = WNetAddConnection2W(
                &resource,
                pass.as_ref().map(|w| PCWSTR::from_raw(w.as_ptr())).unwrap_or(PCWSTR::null()),
                user.as_ref().map(|w| PCWSTR::from_raw(w.as_ptr())).unwrap_or(PCWSTR::null()),
                NET_CONNECT_FLAGS(0),
            );
...
pub fn unmap_shared_directories(mappers: &[MapperSpec]) -> Vec<String> {
    unsafe {
        ...
        for m in mappers {
            let local = to_wide(&m.local_path);
            let result = WNetCancelConnection2W(PCWSTR::from_raw(local.as_ptr()), NET_CONNECT_FLAGS(0), true);
            if result != ERROR_SUCCESS {
                errors.push(format!(
...
pub fn unzip_to_dir(zip_path: &str, target_dir: &str) -> Result<(), String> {
    let file = std::fs::File::open(zip_path).map_err(|e| format!("cannot open zip '{0}': {e}", zip_path))?;
    let mut archive = zip::ZipArchive::new(file).map_err(|e| format!("invalid zip '{0}': {e}", zip_path))?;
    // canonicalize 可能带 \\?\ 前缀，比较时必须统一基准（直接用其拼接出绝对目标路径）
    let canon_base = std::fs::canonicalize(target_dir).unwrap_or_else(|_| PathBuf::from(target_dir));
    let mut total_written: u64 = 0;
    for i in 0..archive.len() {
        let mut entry = archive.by_index(i).map_err(|e| format!("cannot read zip entry #{0} in '{1}': {e}", i, zip_path))?;
        ...
        if !out_path.starts_with(&canon_base) {
            return Err(format!("zip entry '{0}' escapes target directory '{1}'", name, target_dir));
        }
        if let Some(parent) = out_path.parent() {
            let _ = std::fs::create_dir_all(parent);
        }
        let mut out = std::fs::File::create(&out_path)
            .map_err(|e| format!("cannot create '{0}' while extracting '{1}': {e}", out_path.display(), name))?;
        let written = std::io::copy(&mut entry, &mut out)
            .map_err(|e| format!("failed writing '{0}' while extracting '{1}': {e}", out_path.display(), name))?;
        total_written = total_written.saturating_add(written);
        if total_written > UNZIP_TOTAL_LIMIT {
            return Err(format!("zip '{0}' expands beyond the {1} GiB safety limit (possible zip bomb)", zip_path, UNZIP_TOTAL_LIMIT / 1024 / 1024 / 1024));
        }
    }
    Ok(())
}
